...
While listening, Rio shows a compact live meeting-audio input level and the elapsed meeting offset reached by finalized transcription. This lets the user verify long-session progress without exposing live transcript text. An unexpectedly silent or completed capture stream is an interruption, never a successful end while the session still claims to be listening. Rio attempts bounded same-meeting recovery without resetting transcription context; if continuity cannot be restored, it stops explicitly and saves the continuous transcript prefix as incomplete. If bounded capture or transcription backpressure would skip audio, Rio follows the same explicit incomplete-transcript behavior. Once insights appear, the main window expands into a useful live workspace and the bounded, scrollable list uses the available window area. Insight cards are ordered by their last-changed time with the newest card first, and new or updated cards return the view to that latest item. The main window has one primary action: start listening or stop listening and clear the active session. Recent Meetings opens read-only transcripts and insights saved locally from the last two days. Saved transcript segments retain meeting-relative timestamps and can be filtered locally by text so a long completed meeting remains navigable; this is not AI search, editing, or export. A concise cue explains that audio is never retained and finalized transcript text is kept only for that two-day window.
