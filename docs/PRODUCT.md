...
When the app loads and before listening begins, Rio checks microphone access, speech recognition and its required assets, Apple Intelligence, and locale support. It shows all unavailable prerequisites together with the macOS action needed to resolve each one, so the user can fix the setup before trying again. When Apple Intelligence is disabled, Rio identifies the Mac’s preferred language and gives specific guidance when it is not English (US). macOS does not expose the Siri language to third-party apps, so Rio asks the user to verify that Siri uses English (US) in System Settings; it does not change a system setting itself. Apple Intelligence may be unavailable because the Mac is ineligible, the feature is disabled, required assets are not ready, the Mac and Siri languages do not match, organization policy restricts access, or the locale is unsupported. The MVP explains the condition and does not offer a cloud-model fallback.
